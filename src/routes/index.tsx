...
export default function NeonPathGame() {
    ...
    const generateLevel = useCallback((lvl: number) => {
        ...
        const count = Math.min(6 + Math.floor(lvl / 1.5), 24);
    ...
    const handleNodeClick = async (clickedNode: PathNode) => {
        ...
        if (isBlocked) {
            // Shake the node and lose a life
            if (Capacitor.isNativePlatform()) Haptics.impact({ style: ImpactStyle.Heavy });

            setNodes(prev => prev.map(n =>
                n.id === clickedNode.id ? { ...n, isError: true } : n
            ));

            setTimeout(() => {
                setNodes(prev => prev.map(n => ({ ...n, isError: false })));
            }, 500);

            if (lives > 1) {
                setLives(prev => prev - 1);
                toast.error("Path Blocked!");
            } else {
        ...
        setNodes(prev => prev.map(n =>
            n.id === clickedNode.id ? { ...n, cleared: true } : n
        ));
    ...
    const handleWin = async () => {
        ...
        toast.success(`Level ${level} Cleared! +$${reward}`, { icon: '✨' });
    ...
    const handleHint = async () => {
        ...
        if (ad.success) {
            // Find a node that IS NOT blocked
            const unblockedNode = nodes.find(n => {
            ...
            if (unblockedNode) {
                handleNodeClick(unblockedNode);
                toast.success("Hint: Path Cleared!");
            }
        }
    };
            ...
            <div className="h-screen w-full bg-[#050510] flex flex-col items-center justify-center p-8 text-white relative">
                ...
                <div className="relative z-10 w-full max-w-sm flex flex-col items-center text-center">
                    <div className="w-24 h-24 bg-cyan-400 rounded-3xl flex items-center justify-center shadow-glow mb-6">
                        <ArrowIcon direction="UP" className="text-black h-12 w-12" />
                    </div>
                    <h1 className="text-5xl font-black italic mb-2 text-white">NEON PATH</h1>
                    ...
                    <form onSubmit={handleAuth} className="w-full space-y-3">
                        {!isLogin && (
                            <input type="text" placeholder="Gamer Tag" className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-4 font-bold" value={usernameInput} onChange={e => setUsernameInput(e.target.value)} required />
                        )}
                        ...
                        <button type="submit" disabled={isSubmitting} className="w-full bg-cyan-400 text-black py-5 rounded-3xl font-black uppercase tracking-widest mt-4 shadow-glow flex items-center justify-center gap-2">
                            {isSubmitting && <Loader2 className="h-5 w-5 animate-spin" />}
                            {isLogin ? 'Login' : 'Create Account'}
                        </button>
                        <button type="button" onClick={() => setIsLogin(!isLogin)} className="w-full text-center text-[10px] text-white/20 font-black uppercase mt-6 tracking-widest">
                            {isLogin ? "Need an account? Join" : "Have an account? Login"}
                        </button>
                    </form>
                </div>
            </div>
            ...
            <div className="flex-1 w-full max-w-md flex flex-col items-center z-10 px-4 pt-10 pb-32">
                    ...
                    <>
                        <div className="w-full flex justify-between items-start mb-8">
                            ...
                            <div className="flex flex-col items-end gap-2">
                                <div className="bg-white/5 border border-white/10 px-6 py-3 rounded-3xl text-right">
                                    <div className="text-[9px] uppercase font-black opacity-30 tracking-widest">Balance</div>
                                    <div className="text-2xl font-black italic text-cyan-400">${(profile?.cash_balance || 0).toFixed(2)}</div>
                                </div>
                                <div className="text-xs font-black text-white/40 uppercase tracking-widest">Level {level}</div>
                            </div>
                        </div>
                                ...
                                {nodes.map((node) => (
                                    <motion.button
                                        ...
                                        style={{
                                            ...
                                            color: node.color,
                                            filter: node.cleared ? 'none' : `drop-shadow(0 0 10px ${node.color}aa)`
                                        }}
                                        className={cn(
                        ...
                        <div className="flex gap-4 w-full px-4 mt-8">
                             <button onClick={handleHint} className="flex-1 py-5 bg-yellow-400 text-black rounded-3xl flex items-center justify-center gap-2 font-black uppercase text-sm shadow-glow">
                                <Lightbulb size={20} fill="black" /> Hint
                            </button>
                            <button onClick={() => generateLevel(level)} className="p-5 bg-white/5 border border-white/10 rounded-3xl text-white">
                                <RotateCcw size={24} />
                            </button>
                        </div>
                    </>
                    ...
                    <div className="w-full py-8 text-center">
                         ...
                         <div className="bg-white/5 border border-white/10 p-8 rounded-[40px] flex justify-between items-center">
                            <div className="text-left">
                                <span className="font-black text-xl uppercase block">Level Skip</span>
                                <span className="text-[10px] opacity-40 font-bold uppercase">Stuck? Skip it!</span>
                            </div>
                ...
                {activeTab === 'payout' && (
                    <div className="w-full py-8">
                        <h2 className="text-4xl font-black italic uppercase text-center mb-8">Rewards</h2>
                        <div className="bg-gradient-to-br from-cyan-600 to-blue-900 p-8 rounded-[3rem] border border-white/10 shadow-2xl">
                             <div className="flex justify-between items-start mb-6">
                                <Gift size={40} />
                                <div className="text-right">
                                    <div className="text-[8px] uppercase font-black opacity-60">Your Balance</div>
                                    <div className="text-2xl font-black italic">${(profile?.cash_balance || 0).toFixed(2)}</div>
                                </div>
                             </div>
                             <div className="space-y-2">
                                {REWARDS.map(r => (
                                    <button key={r.id} onClick={() => handlePayoutRequest(r)} className="w-full bg-black/20 hover:bg-black/40 p-5 rounded-2xl flex justify-between items-center transition-all border border-white/5">
            ...
            <AnimatePresence>
                {isWon && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-2xl flex flex-col items-center justify-center p-12 text-center">
                        ...
                        <button onClick={() => setLevel(prev => prev + 1)} className="w-full max-w-xs py-6 bg-cyan-400 text-black rounded-full font-black text-xl uppercase shadow-glow flex items-center justify-center gap-3">
                            Next Level <FastForward fill="black" />
                        </button>
                    </motion.div>
                )}
            </AnimatePresence>
